...
#       
#       # Forecast Points (Replaced dotted line with points)
...
# CUSTOM REP PLOTS: Generate comparisons on a single model
generate_single_REP_plots <- function(replist, output_dir, model_name) {
  
  # Source the script locally so parallel workers can access it
  source("SSplotComparisonsREP.R", local = TRUE)
  
  # Place the replist in a named list
  models_list <- list(replist)
  names(models_list) <- model_name
  
  # Summarise the single-model list
  mod.sum <- r4ss::SSsummarize(models_list)
  
  # Safely extract Natural Mortality matching the DPIRD plots logic
  NatM <- NA
  m_match <- grep("NatM", replist$parameters$Label)
  if (length(m_match) > 0) {
    NatM <- replist$parameters$Value[m_match[1]]
  } else {
    warning("Could not find Natural Mortality parameter; defaulting to 0.1 for REP plots.")
    NatM <- 0.1 
  }
  ...
  target_plotdir <- file.path(output_dir, "REP")
  if (!dir.exists(target_plotdir)) {
    dir.create(target_plotdir, recursive = TRUE)
  }
  
  # Run the plotting function
  SSplotComparisons_REP(
    summaryoutput = mod.sum,
    endyrvec = replist$endyr,      # Dynamically extracts the model's end year
    plot = FALSE,                   
    print = TRUE,                  
    plotdir = target_plotdir,
    btarg = 0.4,
    minbthresh = 0.2,
    filenameprefix = "",           # Blank prefix as requested
    legend = TRUE,
    Ftarg = Ftarg_val,
    Fthresh = Fthresh_val,
    Flim = Flim_val,
    legendlabels = c(model_name),
    png = TRUE
  )
}
...
server <- function(input, output, session) {
  ...
  observe({
    withCallingHandlers({
      ...
      observeEvent(input$sector_select_bio_age, {
        rv$bio_age_sectors <- input$sector_select_bio_age
      })
              ...
              process_next_tab <- function() {
                ...
                shinyjs::delay(ms = tab_update_delay_ms, {
                  ...
                  if (tab_to_update == "catch_tab") {
                    ...
                    safe_update_input("updatePickerInput", "sector_select_bio_age", choices_arg_name = "selected")
      ...
      observeEvent(input$run_batch_ss3_btn, {
        ...
        results <- foreach(
          ...
          .export = c('generate_DPIRD_plots', 'generate_custom_RAR_plots', 'generate_single_REP_plots'),
          .errorhandling = 'pass'
        ) %dopar% {
          ...
          tryCatch({
            ...
            tryCatch({
              ...
              log_output <- c(log_output, paste("Worker", i, "generated custom RAR plots."))
            }, error = function(e_rar) {
              log_output <<- c(log_output, paste("Worker", i, "RAR Plot Error:", e_rar$message))
            })
            # ---------------------------------------
            
            # --- SINGLE MODEL REP PLOTS ---
            tryCatch({
              generate_single_REP_plots(replist, output_dir, zip_basename)
              log_output <- c(log_output, paste("Worker", i, "generated single REP plots."))
            }, error = function(e_rep) {
              log_output <<- c(log_output, paste("Worker", i, "REP Plot Error:", e_rep$message))
            })
            # ---------------------------------------
      ...
      observeEvent(input$run_uploaded_ss3_btn, {
        ...
        tryCatch({
          ...
          tryCatch({
            append_to_log("Generating single model REP plots...")
            generate_single_REP_plots(replist, output_dir, zip_basename)
            append_to_log("Single model REP plots generated successfully.")
          }, error = function(e) {
            append_to_log(paste("Error generating single model REP plots:", e$message))
          })
